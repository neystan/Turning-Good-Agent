...
export function ContextWindowIndicator({ context }: { context: ContextWindow | null }) {
  ...
  const label = view.available
    ? `上下文窗口：${view.currentLabel} / ${view.limitLabel} tokens，已用 ${view.usedPercentLabel}，剩余 ${view.remainingPercentLabel}`
    : "上下文窗口：暂无已保存上下文";
  ...
  return <span className={`context-window-indicator is-${view.tone}`} tabIndex={0} aria-label={label} aria-describedby={tooltipId}>
    ...
    <span className="context-window-tooltip" id={tooltipId} role="tooltip">
      <span>上下文窗口：</span>
      {view.available ? <><strong>{view.currentLabel} / {view.limitLabel} tokens</strong><small>已用 {view.usedPercentLabel}，剩余 {view.remainingPercentLabel}</small></> : <small>暂无已保存上下文</small>}
    </span>
  </span>;
}
