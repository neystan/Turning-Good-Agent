...
export function ChannelAccountSettings() {
  ...
  return <>
    <section className="settings-group channel-account-settings" aria-label="消息渠道">
      <div className="channel-account-heading"><div><h3>添加渠道</h3><p>二维码仅在本机控制台短时显示；凭据和平台用户标识不会进入浏览器。</p></div><button className="settings-icon-button" type="button" onClick={() => void load()} aria-label="刷新消息渠道"><RefreshCw size={15} aria-hidden="true" /></button></div>
