...
import { ArchiveRestore, ArrowUp, Check, ChevronDown, Hand, Paperclip, Square, TriangleAlert } from "lucide-react";
...
import { PendingAttachmentCards } from "./AttachmentCards";
...
export function Composer({ session, running, actionsEnabled, segments, autoApprove, multiAgentMode, multiAgentEnabled, multiAgentDisabledReason, multiAgentLocked, inputLocked, contextWindow, restoreFocusVersion, onSegmentsChange, onSend, onStop, onRestore, onAutoApproveChange, onMultiAgentModeChange, onSlashRead, attachments, supportsVision, onFilesSelected, onRemoveAttachment, rootRef }: ComposerProps) {
  ...
  const empty = segments.every((segment) => segment.type === "text" && !segment.text) && attachments.length === 0;
  const blockedVision = attachments.some((item) => item.source === "image") && !supportsVision;
  ...
  return <footer ref={rootRef} className="composer"><SlashCommandMenu slashToken={slashToken} onSelect={selectSlash} /><input ref={fileInputRef} data-testid="attachment-input" type="file" multiple hidden accept=".pdf,.docx,.xlsx,.pptx,.txt,.md,.csv,.json,.png,.jpg,.jpeg,.webp" onChange={(event) => { onFilesSelected(Array.from(event.target.files || [])); event.currentTarget.value = ""; }} /><PendingAttachmentCards items={attachments} onRemove={onRemoveAttachment} /><div className={`composer-input ${blockedVision ? "has-attachment-error" : ""}`} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); onFilesSelected(Array.from(event.dataTransfer.files)); }}>{empty && <span className="composer-placeholder" aria-hidden="true">{placeholder}</span>}<div ref={editorRef} className="composer-editor" aria-label="消息内容" aria-multiline="true" aria-disabled={!editorEnabled} aria-describedby={guidance.length ? "composer-selected-guidance" : undefined} contentEditable={editorEnabled} suppressContentEditableWarning role="textbox" tabIndex={0} onInput={(event) => { const next = readSegments(); const text = event.currentTarget.innerText.replaceAll(ZERO_WIDTH_SPACE, ""); const textSegment = [...next].reverse().find((segment) => segment.type === "text"); if (textSegment?.type === "text") latestCaret.current = { segmentId: textSegment.id, offset: textSegment.text.length }; internalEdit.current = true; onSegmentsChange(next); setSlashToken(text.match(/(?:^|\s)(\/\S*)$/)?.[1] || null); }} onKeyUp={refreshSlashToken} onClick={refreshSlashToken} onPaste={onPaste} onScroll={updateScrollThumb} onKeyDown={onKeyDown} />{guidance.length > 0 && <span id="composer-selected-guidance" className="sr-only">{guidance.map((segment) => `${segment.entry.kind === "mcp" ? "MCP" : "Skill"}：${segment.entry.label}`).join("，")}</span>}{blockedVision && <span className="attachment-capability-error">当前模型不支持图片理解</span>}{scrollThumb && <span className="composer-scroll-thumb" style={{ height: scrollThumb.height, transform: `translateY(${scrollThumb.top}px)` }} />}</div><div className="composer-toolbar"><button className="icon-button" type="button" aria-label="添加附件" title="添加附件" disabled={!editorEnabled} onClick={() => fileInputRef.current?.click()}><Paperclip size={15} /></button><PermissionMenu autoApprove={autoApprove} onChange={onAutoApproveChange} /><MultiAgentModeSelector value={multiAgentMode} enabled={multiAgentEnabled} disabledReason={multiAgentDisabledReason} locked={multiAgentLocked} onChange={onMultiAgentModeChange} /><span className="composer-spacer" /><ContextWindowIndicator context={contextWindow} />{running ? <button className="composer-action is-stop" type="button" aria-label="停止任务" title={connectionHint} disabled={!actionsEnabled} onClick={onStop}><Square size={9} fill="currentColor" strokeWidth={0} aria-hidden="true" /></button> : <button className="composer-action is-send" type="button" aria-label="发送消息" title={connectionHint} disabled={!editorEnabled || blockedVision || attachments.some((item) => item.status === "uploading" || (item.status === "failed" && !item.metadata && !item.retryable))} onClick={onSend}><ArrowUp size={13} strokeWidth={2.5} aria-hidden="true" /></button>}</div></footer>;
}
