...
function PermissionMenu({ autoApprove, onChange }: { autoApprove: boolean; onChange: (enabled: boolean) => void }) {
  ...
  return <DropdownMenu.Root><DropdownMenu.Trigger asChild><button className={`permission-trigger ${autoApprove ? "is-warning" : ""}`} aria-label={`工具权限：${label}`}><Hand size={16} aria-hidden="true" /><span>{label}</span><ChevronDown size={14} aria-hidden="true" /></button></DropdownMenu.Trigger><DropdownMenu.Portal><DropdownMenu.Content className="permission-menu" side="top" align="start" sideOffset={8}><DropdownMenu.RadioGroup value={autoApprove ? "auto" : "default"} onValueChange={(value) => onChange(value === "auto")}><DropdownMenu.RadioItem value="default"><span>默认权限</span><DropdownMenu.ItemIndicator className="permission-indicator"><Check size={15} aria-hidden="true" /></DropdownMenu.ItemIndicator></DropdownMenu.RadioItem><DropdownMenu.RadioItem className="is-warning" value="auto"><TriangleAlert size={15} aria-hidden="true" /><span>自动批准</span><DropdownMenu.ItemIndicator className="permission-indicator"><Check size={15} aria-hidden="true" /></DropdownMenu.ItemIndicator></DropdownMenu.RadioItem></DropdownMenu.RadioGroup></DropdownMenu.Content></DropdownMenu.Portal></DropdownMenu.Root>;
}
