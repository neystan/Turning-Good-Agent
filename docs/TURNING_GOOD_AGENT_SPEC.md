...
> 状态：MVP 已可运行，真实 LLM SDK 化、基础 tool calling、LLM 摘要压缩与 CLI 流式输出已接入，下一步继续补 observability 与 MCP。
...
如果本轮触发 COMPACT，摘要 LLM 调用产生的 `usage` 会并入同一条 `token_usage.jsonl`，因此这一轮账本包含“正常回答 + 摘要生成”的总消耗。
