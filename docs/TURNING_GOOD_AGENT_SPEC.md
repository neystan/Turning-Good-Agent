...
> 状态：MVP 已可运行；Phase 3 Hooks、Phase 4 MCP Client/后台 Worker、Phase 5 Skills、Phase 6 本机 Web 工作台与 Phase 7 CLI 主动能力/长期记忆已实现。
...
- Phase 7 主动能力：同进程 Cron、Breakbeat、Dream、Skill Draft 自进化、系统 incident、在线即时 Fanout 与 CLI 异步显示
- Phase 7 长期记忆：`USER.md`、`SOUL.md`、有界原文证据、Dream revision/登记表和固定顺序完整注入
...
Phase 7 已将长期记忆分为三个职责明确的层：

- `USER.md`：用户的持久事实、稳定偏好、明确纠正、长期目标和长期上下文。
- `SOUL.md`：Agent 已验证的项目约束、工具经验、可复用判断和做事方式。
- 主动事件状态：Breakbeat 事项、Dream 的调度/游标/累计 usage、Observation、Cron 与 incident，独立位于 `<data_dir>/proactive/`，不进入聊天上下文；已有的旧版 `pending_deliveries.json` 保持不变且不是运行时状态。

`USER.md` 最多 12,000 tokens，`SOUL.md` 最多 4,000 tokens，总量最多 16,000 tokens。第一版不做检索/RAG，而是每轮完整注入两份文件；Dream 只从原始 `messages.jsonl` 提炼有来源的证据，禁止用前台压缩摘要替代原文。明确用户纠正、已验证事实、重复行为和模型推断的冲突强度依次递减；未经验证的模型推断不能写入画像或覆盖既有记录。完整规则见 `docs/phases/2026-06-15-phase-7-proactive-memory.md`。
...
- `SOUL` / `USER`：来自 `<data_dir>/memory/SOUL.md` 与 `USER.md`，分别保存 Agent 和用户长期记忆。
...
- skills 内容
- long-term memory retrieval/RAG（后续阶段；当前完整注入）
...
Phase 7 主动能力只由 `tga gateway` 装配。Gateway 持有唯一 `AgentRuntime`、Provider、Tool Catalog、MCP Manager、Skill Manager、共享 `AsyncMessageBus` 与 `ProactiveService`；`tga chat [--session <conversation>]` 是认证 WebSocket Client，省略参数时连接 `default`，Web 是同一 Gateway 内服务。后台任务不进入 `AgentRuntime.run_turn()`，不占前台会话槽，也不写入普通聊天消息、摘要或 Context。

- Cron：LLM 通过 `create_cron`、`list_crons`、`delete_cron` 管理标准五字段的周期性/一次性任务；每次触发先快照当前 Runtime Tool Catalog，再应用后台安全过滤并排除已安装的主动 Tool；全局默认时区为 `Asia/Shanghai`，同一 Job 重叠直接跳过且不补跑。
- Breakbeat：内置单例按每 session 游标和 60 分钟/100k 原文窗口审阅未完成事项；只有用户明确请求时才由 `run_breakbeat` 手动运行，另有 list/delete。
- Dream：内置单例按 24 小时/100k 原文窗口提炼并写入 `USER.md`/`SOUL.md`；工具为 `run_dream` 与 `read_profile_memory`，实际画像变更才通知。
- Skill 自进化：每 10 个完整成功轮次，或小模型识别出明确学习信号时，提炼严格的四类 Observation；每日按 kind 串行批次审阅，回读原文后生成 `.skills/.drafts`，从不自动发布。
- 系统 incident：仅监视 MCP 最终失败/恢复、主动 Job/持久化/投递失败/恢复和主动基础设施失败/恢复；长运行和普通 turn/Tool 业务错误不报警。

领域状态先成功落盘，随后 `NotificationFanout` 按 `origin` 或 `all_subscribed` 展开定向消息，并只立即发布给在线 Web 与 CLI 收件人；离线或未知 Channel 跳过，不重试、不补发。可靠的未来外部 Adapter 投递尚未决定，必须以真实的平台确认机制另行设计。已有的旧版 `pending_deliveries.json` 保持不变且不被读取、迁移、删除或使用。Web 主动结果只进入 `#proactive` 概览或对应领域页和应用内短暂 notice。审批 Tool 在后台默认拒绝，非审批 Tool 仍经过既有安全预检和 `ToolExecutor` 二次检查。`delivery_channels` 仅是 Cron 快照兼容字段，不筛选 Fanout 收件人；飞书实际 Adapter、RAG 和自动发布 Draft 仍不属于当前实现。
...
7. Phase 7：主动能力与长期记忆（已完成；唯一 Gateway）
8. Phase 8：飞书与个人微信 IM Channel（计划中）
9. Phase 9：Multi-Agent 协作模式
