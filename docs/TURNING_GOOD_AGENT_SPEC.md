...
> 状态：MVP 已可运行，真实 LLM SDK 化、基础 tool calling、LLM 摘要压缩、CLI 流式输出、Phase 3 四个轻量 Hooks 与 Phase 4 MCP Client 已完成。
...
- Phase 4 MCP Client：stdio、Streamable HTTP、Catalog、显式 Tool 注册与当前轮附件
...
- `mcp`：来自 MCP Client 的内存 Catalog；默认不注入消息上下文，显式启用的 MCP Tool 只进入 `openai_tools`。
...
`openai_tools` 是与消息列表分离的 API 参数。当前 MCP Catalog、Resource、Prompt 和后续 skills 不会默认进入根 system prompt；只有经过控制 Tool 产生并通过校验的 `ContextAttachment` 会追加到当前 AgentLoop working messages。

预算规则：

- 最终发给模型的上下文受 `max_context_tokens = 300000` 约束。
- `BUILD` 不裁剪 `uncompacted_history`，未压缩历史在压缩前必须完整注入。
- 如果 `system prompt + long memory + summary + uncompacted_history + current input + openai_tools` 超过 `max_context_tokens`，当前策略是拒绝本轮并提示上下文过大。
- `BUILD` 和 `SAVE` 的上下文预算由 `context/token_budget.py` 统一计算，覆盖 `SYSTEM_PROMPT`、长期偏好、summary、一次实际 `openai_tools` 序列化内容、未压缩历史和当前用户输入。
- `recent_window` 只在 `COMPACT` 需要压缩时临时计算。
- `recent_window` 受 `recent_window_token_limit = 20000` 约束，并在压缩后写入 `session.uncompacted_history`。
- system prompt、long memory、summary、uncompacted history、current input 和实际 `openai_tools` 都应计入上下文预算；后续 skills/MCP 内容只有实际注入消息时才计入。
...
- initialize、initialized、Tools/Resources/Resource Templates/Prompts 的分页发现、tools/call、resources/read 与 prompts/get。
