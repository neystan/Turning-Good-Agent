...
| 路径 | 作用 |
| --- | --- |
| `web/backend/app.py` | 创建 FastAPI lifespan、REST API、WebSocket endpoint 和本地静态资源托管；动作回包使用 `client_action_id` 关联乐观消息错误，并提供只读上下文窗口摘要。 |
| `web/backend/coordinator.py` | 调度单会话 turn、guidance、Stop、审批 Future、全局并发槽位和 MessageBus 消费；为有限动作保留受理回执，避免 Web 重试重复入队。 |
...
Web Host 的 `runtime.start()` / `runtime.close()` 只在 FastAPI lifespan 中调用一次。REST 只读取既有会话与观测文件，WebSocket 只传输内存事件，不新增 JSONL；草稿只存在浏览器，首条非空消息才创建 session。浏览器以 `client_action_id` 对齐乐观消息与错误、以 `after_event_id` 重订阅；Coordinator 缓存有限受理回执，避免连接恢复或手动重试重复创建 turn。运行中 guidance 成功排队后通过既有 `task.status` 在当前 turn 显示“已引导”，不新增 `guidance.consumed`。断线未完成轮次、重试和被替代的旧展示记录只存在浏览器标签页缓存，不改变 Runtime 上下文、Session JSON/JSONL 或 SAVE 唯一持久化边界。
