...
2026-07-26 工作台重构收口：WebSocket 动作使用 `client_action_id` 关联乐观消息与错误，前端以历史加载版本闸门、有限退避重连和 `after_event_id` 回放保持会话稳定；会话栏仅按置顶优先排序，三点操作菜单通过 Portal 脱离侧栏滚动裁切，并支持键盘可用的会话搜索。聊天区与侧栏独立滚动，短 user 消息按内容宽度显示；检查器覆盖右侧时不移动消息列，只单独收窄 Composer。真实事件按 `request_id` 显示为可折叠“思考中”活动簇，运行中 guidance 立即显示为 user 消息并由既有 `task.status` 标记“已引导”。检查器先展示累计 token、当前上下文、压缩次数与工具失败，再按需读取原始记录。此收口不新增 JSONL，也不新增 `guidance.consumed`，且“思考中”不表示或暴露模型内部思维链。当前环境无 Chromium，Playwright 截图验证待后续具备浏览器的环境补充。
...
- 全局自动批准按钮位于输入区左下角，默认关闭；它影响所有会话的后续审批点。
...
- 所有图标使用 `lucide-react`，有 tooltip、键盘焦点和可访问标签。
