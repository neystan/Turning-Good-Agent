...
2026-07-26 工作台重构收口：WebSocket 动作使用 `client_action_id` 关联乐观消息与错误，前端以历史加载版本闸门、有限退避重连和 `after_event_id` 回放保持会话稳定；会话栏仅按置顶优先排序，聊天区与侧栏独立滚动。真实事件按 `request_id` 显示为可折叠“思考中”步骤，运行中 guidance 立即显示为 user 消息并由既有 `task.status` 标记“已引导”。检查器先展示累计 token、当前上下文、压缩次数与工具失败，再按需读取原始记录。此收口不新增 JSONL，也不新增 `guidance.consumed`，且“思考中”不表示或暴露模型内部思维链。
