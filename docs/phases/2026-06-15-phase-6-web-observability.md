...
会话操作菜单使用紧凑图标布局，并保留可访问名称；不会显示鼠标悬浮文字。检查器的累计指标与分类组采用连续信息面，只有展开的单条记录与原始 JSON 进入更深的圆角层级，避免观测面板成为卡片堆叠。
...
- 会话操作菜单与侧栏共用底色，采用紧凑纯图标布局；视觉文字隐藏但语义文本保留给读屏软件。菜单项仅在直接悬停或键盘高亮时出现阴影。
