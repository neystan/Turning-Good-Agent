...
- 外部 Skill 直接复制或下载到 `skills/<skill-name>/`，不设置第二个外部目录、不做 Git URL 拉取、ZIP 安装、Marketplace 或自动更新。
...
以下三个 Tool 全部注册进既有 `ToolRegistry`：
...
- 多目录、远程下载、Git URL、ZIP 安装、Marketplace、签名、版本同步或自动更新。
