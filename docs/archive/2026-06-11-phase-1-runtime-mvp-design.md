...
`token_count` 记录消息自身内容的 tokenizer token 权重，用于短期压缩窗口计算；LLM SDK 返回的真实 usage 单独写入 `true_token_usage.jsonl`。
...
压缩相关的外部观测字段不再写进 `session.json` 或 `true_token_usage.jsonl`，而是只出现在：
